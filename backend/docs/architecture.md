...
```text
              ...
              │                          │   /api/requerimiento/embarques
              └──────────────┬───────────┘   /api/requerimiento/remision-detalle
...
| `routers/requerimiento.py` | ~224 | Vista Calendario de requerimiento (fase 1 + 2) — `GET /api/requerimiento/calendario` (`ventana`, `fecha_max`, `universo`; TTL=300s, maxsize=32, key=(universo, ventana, fecha_max)), `GET /api/requerimiento/orden-detalle` (`idMaterial` requerido + `cliente`/`ciudad`/`desde`/`hasta`/`forecast`; TTL=120s, maxsize=256), `GET /api/requerimiento/embarques` (fase 2 — `meses_atras`, `universo`; TTL=300s, maxsize=32, key=(universo, meses_atras)) y `GET /api/requerimiento/remision-detalle` (fase 2 — `idMaterial` requerido + `cliente`/`ciudad`/`desde`/`hasta`; TTL=120s, maxsize=256). Los dos endpoints de fase 2 son el espejo hacia el pasado de `/calendario` y `/orden-detalle`. |
...
| `domain/modelo.py` | ~470 | 27 modelos pydantic. Entrada: `FilaListado`, `FilaBom`, `FilaRuta`, `FilaWip`. Salida árbol: `DemandaPT`, `PasoRuta`, `AristaPadre`, `NodoComponente`, `ArbolPT`. Salida Resumen: `BloqueProceso`, `PTEnProceso`, `Planta`. Salida Calendario: `CeldaCalendario` (demanda desagregada por PT × Cliente × Ciudad × Fecha × bForecast), `OrdenLinea` (línea de orden de venta de una celda), `CeldaEmbarque` (fase 2 — remisiones desagregadas por PT × Cliente × Ciudad × Fecha, espejo hacia el pasado de `CeldaCalendario`), `RemisionLinea` (fase 2 — línea de remisión de una celda de embarque, espejo de `OrdenLinea`). Base con `extra="ignore"`. |
...
| `domain/db.py` | ~540 | Lectores SQL: `fetch_listado`, `fetch_detalle` (multi-resultset), `fetch_bloques`, `fetch_pts_en_proceso`, `fetch_plantas`, `fetch_requerimiento_calendario`, `fetch_orden_detalle`, `fetch_embarques_calendario` (fase 2), `fetch_remision_detalle` (fase 2). Helpers: `_strip_param_declarations`, `_decl_int`, `_ciudades_predicate`, `_tipomat_predicate`, `_clase_predicate`, `_cliente_eq_predicate`, `_ciudad_eq_predicate`, `_fecha_rango_predicate`, `_forecast_predicate`. `ping(conn)` para `/health`. |
...
| `sql/Q_embarques_calendario.sql` | ~91 | Fase 2 — espejo hacia el pasado de `Q_requerimiento_calendario.sql`. Historial de remisiones (`EPS.dbo.vwRemisiones`) desagregado por (PT × Cliente × Ciudad × Fecha[día]), acotado a `hoy - @meses_atras`. Puentes NetSuite→EPS: `Item_Id→idMaterial` (vía `tblDemandaEPS` activa), `Customer_Id→tblCliente.IdNetSuit`, `City→tblCiudad` por nombre normalizado (`UPPER+TRIM`, best-effort ~2% sin match cae en `idCiudad NULL`). Incluye `/*PT_UNIVERSO_FILTER*/`. |
| `sql/Q_remision_detalle.sql` | ~75 | Fase 2 — espejo hacia el pasado de `Q_orden_detalle.sql`. Líneas de remisión de una celda de embarque: `Remision`, `Factura`, `OrdenVenta`, `OrdenCompra`, `Fecha`, `Piezas`, `Precio_Unitario`. Mismos puentes que `Q_embarques_calendario.sql`. Placeholders `/*CLIENTE_FILTER*/`, `/*CIUDAD_FILTER*/`, `/*FECHA_FILTER*/` (reutiliza los predicados de `Q_orden_detalle.sql` sobre el CTE `rem` aliaseado `d`). |
...
- **`requerimiento.py`**: cuatro endpoints, en dos pares espejo (futuro/requerimiento vs pasado/embarques — fase 2). `/calendario` y `/embarques` siguen el patrón de `pts.py` (cache simple keyed por `(universo, ventana, fecha_max)` / `(universo, meses_atras)`, TTL 300s — espejan `/api/pts` porque alimentan el mismo panel/filtros). `/orden-detalle` y `/remision-detalle` son más específicos por celda (`idMaterial` + cliente/ciudad/rango[/forecast]), TTL 120s más corto y `maxsize` más alto (256) porque la combinatoria de celdas es grande pero cada hit es barato.
