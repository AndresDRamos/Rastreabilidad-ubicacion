...
| BOM | `EPS.AppProc.tblBomExplosionado` | Árbol multinivel (IdBomParent, IdPadre, BomLevel, CantidadEnsamble). Filtro `IdTipoMaterial IN (1,3)` excluye MP/Dibujos/Indirectos/Herramental. |
...
| WIP | `EPS.Produccion.tblEtiqueta` (`bActiva=1, idEstatusEtiqueta=2, idTipoEtiqueta=3`) | Etiquetas LIBERADAS activas, indexadas por `(idMaterial, idProcesoSiguiente)`. |
...
### Interpretación crítica del WIP
...
Internamente hace dos pasadas sobre la estructura combinada.
...
**(b) Para intermedios**: agregar un `PasoRuta` virtual al final con `idProceso=almacen_wip_id (16)`, `es_virtual=True`. Representa el buffer donde el componente espera consumo por el padre. Su `wip_en_paso = wip[(idComp, 16)]`. **No se renderiza como nodo** en el canvas; alimenta el `wipBuffer` de la card del componente.
...
| 11 | Multi-planta dentro de una ruta es normal (caso PT 49178: P4→P4→P5→P4→P1). | Ignorar planta para el árbol; mostrar como dato informativo en UI. |
...
- **No considera tiempos**: `tblMaterialRutaTiempo.TiempoProceso` existe en el response (`PasoRuta.idPlanta`, sin cargar el tiempo hoy), pero no se usa para programación. Si en el futuro se quiere mostrar ETA, hay que agregar el campo a `FilaRuta` y propagarlo.
- **No re-calcula al editar etiquetas**: si un operador captura/elimina una etiqueta WIP, el frontend no recibe push. Hay que refrescar manualmente (tab close + reopen) para invalidar el cache de TanStack.
