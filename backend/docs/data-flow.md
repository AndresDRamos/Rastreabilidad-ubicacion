...
| `GET /api/bloques?cliente=&planta=&ciudades=&tipos_material=&clases=&universo=` | `db.fetch_bloques` directo | `Q_bloques.sql` | TTL 120s, maxsize 128 | TanStack staleTime 2 min |
| `GET /api/flujo?cliente=&planta=&ciudades=&tipos_material=&clases=&universo=` | `db.fetch_flujo` directo (2 result-sets) | `Q_flujo.sql` | TTL 120s, maxsize 128 | TanStack staleTime 2 min |
