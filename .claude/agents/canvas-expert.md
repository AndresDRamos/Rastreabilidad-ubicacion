...
6. **Pasos virtuales no se renderizan** como nodos. `buildGraph` filtra `pasosReales = c.ruta.filter(p => !p.es_virtual)` antes de emitir nodos `process`. El WIP del paso virtual (Almacen WIP) ya viene sumado dentro de `wip_total`, así que la card no lo lee aparte.
...
| `component` | `ComponentNode` | 240px | `reqNeto` | `wipTotal` | covered/partial/empty/neutral |
| `process` | `ProcessNode` | 220px | `reqPaso` | pipeline Disp→Recib→Transf | covered/partial (visual de borde) |

`reqNeto = max(0, reqBruto - wipTotal)` viene del backend. Los dos modos de la card son complementarios: `wipTotal + reqNeto = reqBruto`.

> Hasta 2026-07 la card usaba `wipBuffer` / `reqBufferFaltante = max(0, reqBruto - wipBuffer)`, que solo miraban el buffer `Almacen WIP`: un componente cubierto mostraba "0 en buffer" y piezas por fabricar, contradiciendo su propio badge. **No lo reintroduzcas.**
