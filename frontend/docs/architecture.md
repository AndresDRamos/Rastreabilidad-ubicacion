...
| `api/types.ts` | Tipos espejo de los modelos pydantic del backend: `FilaListado`, `DemandaPT`, `PasoRuta`, `AristaPadre`, `NodoComponente`, `ArbolPT`, `BloqueProceso`, `PTEnProceso`, `Planta`, `CeldaCalendario`, `OrdenLinea`, `CeldaEmbarque` (fase 2), `RemisionLinea` (fase 2), `Mode`. Más los tipos propios `CalGranularidad` y `CalModo` de la vista Calendario (sin equivalente pydantic). |
| `api/client.ts` | Axios instance con `baseURL = "/api"` (queries usan rutas tipo `/pts`, `/bloques`, etc., y Vite/uvicorn las resuelven). |
| `api/queries.ts` | Hooks TanStack: `usePts`, `useArbol`, `useBloques`, `usePtsEnProceso`, `usePlantas`, `useRequerimientoCalendario`, `useOrdenDetalle`, `useEmbarquesCalendario` (fase 2), `useRemisionDetalle` (fase 2, entre otros). Cada uno con su `staleTime` apropiado (ver `conventions.md`). |
...
| `components/Sidebar/CalendarioPanel.tsx` | Vista Calendario de requerimiento, fase 1 (demanda futura) + fase 2 (embarques). Lee `useRequerimientoCalendario(ventana, fechaMax, universo)` y `useEmbarquesCalendario(mesesAtras, universo)`, filtra client-side (pt/cliente/ciudad[/forecast], mismo patrón que `PtTable`) y agrupa por (PT × Cliente × Ciudad) — las filas ancla son las de requerimiento; los embarques solo enriquecen filas existentes, salvo en modo `"embarques"` puro donde crean filas nuevas. Bucketiza `Fecha` a día/semana/mes (`calGranularidad`) **en el navegador**, sin re-fetch. Eje de tiempo unificado: columnas de embarque (heatmap azul, antiguo → reciente) + columna Past-due fija (heatmap rojo) + columnas de requerimiento (heatmap cercano cálido → lejano frío; forecast hachurado) + fila Total. Toggle `calModo` (`"requerimiento" \| "embarques" \| "ambos"`, default `"ambos"`) decide qué mitad del eje se muestra. Clic en celda → `setCeldaDetalle({ tipo: "requerimiento" \| "embarque", ... })` → popover inferior con `useOrdenDetalle(...)` (líneas de orden de venta) o `useRemisionDetalle(...)` (líneas de remisión) según `tipo`. |
...
| `CeldaEmbarque` (fase 2 — result-set de `Q_embarques_calendario.sql`) | `CeldaEmbarque` |
| `RemisionLinea` (fase 2 — result-set de `Q_remision_detalle.sql`) | `RemisionLinea` |
