...
13. **Las columnas consolidadas del Excel son definiciones de negocio, no cálculos libres.** Viven en `services/export_service.py` y las fija `tests/unit/test_export.py`: *Total WIP* = `wip_total` (para un kit, unidades ya ensambladas = kits completos en piso); *Total completos* = WIP que espera en `PROCESOS_NO_FABRICACION` (Embarques=13 y Almacen WIP=16) — llegar ahí implica haber pasado toda la ruta; *Requerimiento neto* = `req_neto`. Si cambias una definición, cambia su test — el planner toma decisiones de producción con esas columnas.
13-bis. **"Total completos" NO es el WIP del último proceso de fabricación.** Por la trampa #10, `wip_en_paso[X]` = piezas esperando **entrar** a X: las del último proceso son justo las que aún NO terminaron. Medido en el PT canónico: esa lectura daría "4 completas" para `90358715-RA` (4 pzs a las que les falta Doblez) y "0" para `91711040-RA` (que tiene 9 ya terminadas en el buffer) — invierte la realidad. Fijado por `test_total_completos_no_es_el_wip_del_ultimo_proceso_real`.
